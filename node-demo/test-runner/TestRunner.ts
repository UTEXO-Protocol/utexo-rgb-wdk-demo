...
// In RN these came from 'expo-file-system' + 'react-native'. On Node the
// shim provides drop-in equivalents that write to $WDK_DATA_DIR/reports/
// and report Platform.OS = 'node'.
import { Paths, File, Platform } from './platform-shim'
import type { LnExt } from './LnExt'
import { logEvent, logStore, runWithLogging } from './logger'
...
function platformTag (): 'ios' | 'android' | 'web' | 'node' | 'other' {
  if (Platform.OS === 'ios') return 'ios'
  if (Platform.OS === 'android') return 'android'
  if (Platform.OS === 'web') return 'web'
  if (Platform.OS === 'node') return 'node'
  return 'other'
}
